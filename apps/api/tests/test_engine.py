...
from verdict.integrity import integrity_score, run_all
...
def test_loss_outside_cover_declines():
    c = make_claim(date_of_loss=date(2025, 11, 2), policy=make_policy(inception=date(2025, 1, 1)))
    assert decide(c, TODAY).outcome is Outcome.DECLINE


def test_missing_evidence_requests_rather_than_declines():
    c = make_claim(evidence=[EvidenceItem("claim_form", True), EvidenceItem("damage_photos", True)])
    r = decide(c, TODAY)
...
def test_integrity_flags_escalate_not_decline():
    c = make_claim(integrity=[IntegrityFlag("PHOTO_PREDATES_LOSS", "Captured before the loss.", 3)])
    r = decide(c, TODAY)
...
def test_loss_before_inception_is_serious():
    c = make_claim(date_of_loss=date(2026, 2, 1), policy=make_policy(inception=date(2026, 3, 1)))
    flags = run_all(c)
